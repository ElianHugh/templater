...
test_that("uiElements are all of class shiny.tag", {
    ...
    expect_true("shiny.tag" %in% class(templater_directory(getwd())))
    expect_true("shiny.tag" %in% class(templater_creator()))
})

test_that("templater_ui is of class shiny.tag.list", {
    expect_true("shiny.tag.list" %in% class(templater_ui()))
    expect_is(htmltools::findDependencies(templater_ui()), "list")
})
